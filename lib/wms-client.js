...
async function fetchPutawayTasks({
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    const pagination = payload?.pagination && typeof payload.pagination === "object"
      ? Object.fromEntries(Object.entries(payload.pagination).filter(([, value]) =>
        typeof value === "number"
        || typeof value === "boolean"
        || (typeof value === "string" && /^\d+$/.test(value)),
      ))
      : {};
    pageTrace.push({ requestedPage: page, rowCount: data.length, pagination });
    rows.push(...data);
  ...
  return { rows, complete, pageTrace };
}

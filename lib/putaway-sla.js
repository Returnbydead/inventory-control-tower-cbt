...
const AT_RISK_MINUTES = 40;
const URGENT_MINUTES = 50;
...
function calculateSla({ inProgressAt, completedAt, now = new Date(), isCompleted = false }) {
  const start = asDate(inProgressAt);
...
function normalizePurchaseOrderDetail(detail) {
  ...
  const doneGrHistory = histories.find((entry) =>
    ["COMPLETED", "PARTIALLY_FULFILLED"].includes(
      String(entry?.activity_name || "").trim().toUpperCase(),
    ),
  );
  const receivedAt = asDate(data.received_at)
    || asDate(doneGrHistory?.start_date)
    || asDate(doneGrHistory?.end_date);
  return {

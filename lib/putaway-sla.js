...
function calculateSla({ grnAt, completedAt, now = new Date() }) {
  const start = asDate(grnAt);
  const end = asDate(completedAt) || asDate(now);
  ...
  return {
    ...
    within_sla: Boolean(completedAt) ? elapsedMinutes <= SLA_MINUTES : null,
  };
}

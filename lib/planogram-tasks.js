...
function suggestionLabel(item) {
  ...
  const aisleLabel = clean(item?.aisle_label);
  if (zone && /^\d{2}(?:-\d{2})?$/.test(aisleLabel)) {
    return `${zone} - aisle ${aisleLabel}`;
  }
...
async function fetchPlanogramTaskLedger({
  ...
  const timeout = setTimeout(
    () => controller.abort(new Error(`Ledger task Planogram timeout setelah ${timeoutMs} ms.`)),
    timeoutMs,
  );
  let response;
  try {
...
async function fetchPlanogramAssignmentPreview() {
  const url = new URL(gasUrl());
  url.searchParams.set("action", "planogram_assignment_preview");
  url.searchParams.set("_ts", String(Date.now()));
  ...
  return parseGasResponse(response, "Preview auto assign PIC Planogram");
}

async function postPlanogramAutoAssign() {
  const response = await fetch(gasUrl(), {
    method: "POST",
    redirect: "follow",
    headers: { "content-type": "application/json", accept: "application/json" },
    body: JSON.stringify({ kind: "planogram_auto_assign" }),
  });
  return parseGasResponse(response, "Auto assign PIC Planogram");
}

...
function targetFor(rackName) {
  ...
  const key = `${address.zone}:${String(address.rackSequence).padStart(2, "0")}`;
  ...
  // The SRC1 sequence-18 exception applies only to physical Aisle 13-17,
  // which is the second numeric component in the confirmed rack address.
  for (const category of rule.bay_allowed[String(address.aisle)] || []) {

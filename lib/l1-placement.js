...
function planogramRules(rules) {
  return Array.isArray(rules) ? rules : (gsheetRulesPayload.rules || []);
}

function planogramSource(rules) {
  return planogramRules(rules)[0]?.source || gsheetRulesPayload.source;
}

function gsheetRuleForAddress(address, rules) {
  const allowed = new Set();
  for (const rule of planogramRules(rules)) {
  ...
  return {
    ...
    allowed: [...allowed],
    source: planogramSource(rules),
    address,
  };
}

function targetFor(rackName, rules) {
  ...
  if (PRIMARY_PLANOGRAM_ZONES.has(address.zone)) {
    return gsheetRuleForAddress(address, rules);
  }
...
function placementAreasForCategory(value, rules) {
...
function placementRangesForCategory(value, rules) {
  const category = normalizeCategory(value);
  return planogramRules(rules)
...
function evaluateCategory(rackName, actualCategory, rules) {
  const target = targetFor(rackName, rules);

...
function decodeMaster(masterPayload) {
  ...
  })).filter((row) => row.rack_name && !isExcludedOccupancyRack(row.rack_name));
}
...
function finalise(row) {
  ...
  const sloc_utilization_pct = row.location_count
    ? (row.used_sloc_count / row.location_count) * 100
    : null;
  return {
    ...row,
    ...
    used_sloc_count: Math.round(row.used_sloc_count * 1000) / 1000,
    sloc_utilization_pct: sloc_utilization_pct === null ? null : Math.round(sloc_utilization_pct * 100) / 100,
  };
}
...
function summarizeL1ByStorage(l1Rows) {
  ...
  for (const row of l1Rows) {
    ...
    metrics.used_sloc_count += row.used_sloc_count;
...
function summarizeOccupancy(
  masterPayload,
  liveByRack,
  capacityByKey = DEFAULT_CAPACITY_BY_KEY,
  planogramRules,
) {
  ...
  for (const location of selectedMaster) {
    ...
    if (number(live.qty) > 0) zoneRow.used_sloc_count += 1;

    const target = targetFor(location.rack_name, planogramRules);
    ...
    if (number(live.qty) > 0) l1Row.used_sloc_count += 1;
    ...
    for (const [category, usedQty] of live.qty_by_l1 || []) {
      ...
      const occupiedCategories = live.used_sloc_by_l1 || new Set(live.qty_by_l1?.keys() || []);
      if (occupiedCategories.has(category)) categoryRow.used_sloc_count += 1;
    }
  }

  const unmasteredStock = [...liveByRack.entries()].reduce((total, [rackName, live]) => (
    masterRacks.has(rackName) || isExcludedOccupancyRack(rackName) ? total : total + number(live.qty)
  ), 0);
  ...
  const total = finalise(zones.reduce((summary, row) => ({
    ...
    used_sloc_count: summary.used_sloc_count + row.used_sloc_count,
  }), { storage_type: "Total", zone: "ALL", space_qty: 0, used_qty: 0, wrong_qty: 0, location_count: 0, used_sloc_count: 0 }));
  return {
    total,
    zones,
    l1,

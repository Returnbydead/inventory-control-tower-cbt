...
function placementSuggestions(row, context, limit = 4, planogramRules) {
  if (row.status !== "WRONG_L1") return [];
  const category = normalizeCategory(row.l1_category_name);
  const candidates = placementAreasForCategory(category, planogramRules).map((candidate) => {
...
function buildPlanogramDetailRows(sourceRows, planogramRules) {
  ...
  return rows.map((row) => ({
    ...row,
    suggestions: placementSuggestions(row, context, 4, planogramRules),
  }));
}
